...
using std::cout;
using std::endl; //instead of loading all the std::functions, only these one,
// for avoid possible conflicts
...
template <typename kt, typename vt> class Iterator; //class declaration needed
// so that Tree compiles
...
public:
    ...
    node* right;
    node* up = nullptr;
    node();  //add default constructor senno nn compila ..
  //          serve x classe standad vector
    node (const kt& k = kt(), const vt& v = vt(), node* l = nullptr,
	  node* r = nullptr): key(k),val(v),left(l), right(r) {}
  //costructor con i valori di default
...
node<kt,vt>::node(){    //implementation of the default constructor
    ...
    right=nullptr;
    val = vt();
    key = kt();  //default constructor explicitely use default
    // constructors for the types kt, vt
...
class Tree
{
  ...
  //insert_noiter recalls insert helper (wrapper of insert_helper)
  //i called it insert_noiter because I do not use iteratorsyet.
  
       void clear() {clear_helper(root);}
  //clear calls clear helper starting from the root
...
//key,val,root ..insert helper ricorre dentro se stessa
{
	...
	//the first node has a key, a value and points to nullptr and
	// nullptr left and right
        _size = _size+1;
	// when I create the first node the size of the tree increases
	// and so on...
  //every time i define a newnode a redefine the parent
        
    }
    else if(key < t->key){
      //if the key of the new node is smaller than put it left
        
        t->left = insert_helper(key, val, t->left);
	//cout << "node current L "<< t->key << endl;
        t->left->up = t;
    }
    else if(key > t->key){ //if the key of the new node is bigger
      // than put it left
        
        t->right = insert_helper(key, val, t->right);
	//cout << "node current R "<< t->key << endl;
        t->right->up = t;
    }
    return t;
}
...
template<typename kt, typename vt>
class Iterator
//  remove Tree<kt,vt>::Iterator because is defined outside Tree class
{
private:
